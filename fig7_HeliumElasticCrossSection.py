...
from conversions import BOHR_SQ_TO_ANGSTROM_SQ
...
def loop_over_partial_waves(energy):
    '''
    Summation over partial waves.
    ...
    -------
    TYPE Float
        DESCRIPTION. Total elastic cross section 
    ...
    k_energy = sqrt(energy_sq)
    
    xs_total_i = 0.0
    order_max =  10
    
    for order in range(0, order_max):
        phase_e = integrate_function(k_energy, order)
        l_xs_total_i = ( 4 * pi / (k_energy * k_energy)) * (2.0 * order + 1.0) * (sin(phase_e) * sin(phase_e))
        xs_total_i += l_xs_total_i

    return xs_total_i  * BOHR_SQ_TO_ANGSTROM_SQ


def plot_helium_cross_section():
    '''
    plot total elastic cross section of helium.
    ...
    energy_grid = np.linspace(100, 700, num=7, endpoint=True)           
    for energy in energy_grid:
        cross_section = loop_over_partial_waves(energy)
        energy_cross_section.append(cross_section)
                
    plt.plot(energy_grid, energy_cross_section)
    plt.xlabel('Ecm(eV)')
    plt.ylabel('elastic cross section') 
    plt.show()

#plot total elastic cross section of helium
print('Total elastic cross section')
plot_helium_cross_section()
